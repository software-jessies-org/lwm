...
| `navigate.{h,cc}` | `Direction` and `PickWindowInDirection` — which window Super+arrow moves the focus to, by the cone rule described in the header. Monitors don't come into it: every window is a candidate wherever it is. |
...
| `keyboard.cc` / `keyboard.h` | ~110 | `GrabNavigationKeys` (the passive Super+arrow grabs, on the *root* window — repeated under both lock modifiers, same as the mouse gestures) and `HandleKeyPress`, which turns a press into a focus change via `navigate.h`. Re-run on `MappingNotify`, since a layout switch moves the keycode an arrow sits on. The keysym constants it matches live in `xlib.h`: `keysymdef.h` is Xlib, and so out of bounds. |
...
| `disp_test.cc`, `focus_test.cc`, `client_test.cc`, `ewmh_test.cc`, `drag_test.cc`, `keyboard_test.cc` | The tests that need a server: `EvConfigureRequest`'s Nautilus offset arithmetic, `Focuser`'s history and its timerfd deferral, the client lifecycle across `LScr`'s registries, `fix_stack` plus the recursion guard, the Windows-key gestures from the `ButtonPress` down to the window moving or losing its furniture, and Super+arrow from the `KeyPress` down to the focus moving. `FakeServer::GetKeyboardMapping` reports a keyboard with the four arrows on it and nothing else. |
...
* **A new key binding** → `keyboard.{h,cc}`: add the keysym to `xlib.h`, the
  ...
  own grab. Grabs go on the root window, not per client, so they work over the
  desktop too.
