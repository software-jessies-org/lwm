...
| `client.cc` / `client.h` | ~610 | `Client` methods (geometry, border drawing, raise/lower/close/state, full-screen, maximisation, and the `NotePreExpandRect`/`Unexpand` pair behind the centre-cell double click). Also the resize-feedback popup. `Focuser` moved out to `focus.{h,cc}`. |
