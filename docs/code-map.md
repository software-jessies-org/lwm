...
| `client.cc` / `client.h` | ~720 | `Client` methods (geometry, border drawing, raise/lower/close/state, full-screen, maximisation, the `NotePreExpandRect`/`Unexpand` pair behind the centre-cell double click, and `SetFramed`, which adds or removes the furniture on a live client). Also the resize-feedback popup. `Focuser` moved out to `focus.{h,cc}`. |
...
| `screen.cc` / `screen.h` | ~360 | `LScr`: window/client registry, GCs and colours, EWMH root properties, window-tree scan at start-up, `SetVisibleAreas` (calls into `screenlayout.cc`), and the `Furnish`/`Unfurnish` pair that creates and destroys a client's frame window. Owns `Hider`/`Focuser` by value, so `screen.h` includes `hider.h`/`focus.h`. |
...
| `disp_test.cc`, `focus_test.cc`, `client_test.cc`, `ewmh_test.cc`, `drag_test.cc` | The tests that need a server: `EvConfigureRequest`'s Nautilus offset arithmetic, `Focuser`'s history and its timerfd deferral, the client lifecycle across `LScr`'s registries, `fix_stack` plus the recursion guard, and the Windows-key gestures from the `ButtonPress` down to the window moving or losing its furniture. |
...
* **Which windows get frames** → `ewmh_hasframe` (`ewmh.cc`) and the motif
  ...
  `Client::SetFramed` (`client.cc`) — see "Turning the furniture on and off"
  in `concepts.md` for what reparenting a live client costs.
